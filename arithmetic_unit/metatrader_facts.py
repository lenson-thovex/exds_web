...
multiplyer = float(input('Kontogröße: '))/initial_balance
initial_balance = initial_balance*multiplyer
...
biggest_consecutive_return = win_frame.sum()*multiplyer
biggest_consecutive_loss = loss_frame.sum()*multiplyer
...
net_profit = (df.iloc[-1,8] - df.iloc[0,8])*multiplyer
gross_profit = (df[df['return']>0.0]['return'].sum())*multiplyer
gross_loss = (df[df['return']<0.0]['return'].sum())*multiplyer
max_dd = float(np.max(drawdowns(df)))
...
biggest_profit = max(df['return'])*multiplyer
biggest_loss = min(df['return'])*multiplyer
avg_win = df[df['return']>0.0]['return'].mean()*multiplyer
avg_loss = df[df['return']<0.0]['return'].mean()*multiplyer
